...
要将文章的状态从 `MOD`（已修改）或 `NEW`（新建）变为 `✓`（已同步/未修改），你需要更新项目的 **Hash 记录**。

这个状态检查机制是通过比对文件的“当前内容 Hash 值”与“记录在案的 Hash 值”来实现的。

请在终端执行以下命令：

### 方法一：更新状态（推荐）

这个命令会更新已存在文章的 Hash 记录，并添加新文章的记录。

```bash
make article-update
```

运行后，再次执行 `make list`，你应该能看到所有文章都变成了 `✓`。

### 方法二：完全重新初始化（如果方法一无效）

如果 `article-update` 没有处理 `NEW` 状态的文章，你可以强制重新生成所有文章的 Hash 记录:

```bash
make article-init
```

这会重新扫描 `source/_posts` 下的所有文件，并将它们当前的快照视为“干净”的状态。

### 💡 原理解析

- **NEW**: 表示该文件存在于目录中，但 `.article-hashes` 记录文件中找不到它。
- **MOD**: 表示文件内容（除去 `updated` 时间戳字段）与 `.article-hashes` 中记录的内容不一致。
- **✓**: 表示文件内容与记录完全一致。

当你运行上述命令后，系统会将当前文件的状态写入记录文件，因此系统会认为文件“未被修改”。
